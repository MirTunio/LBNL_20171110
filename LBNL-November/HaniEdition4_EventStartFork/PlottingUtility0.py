...
#                plt.axvline(x=story[i],c='green',alpha=0.7,linewidth=4)
        ...
        plt.ylim(-7,7)#0.30,0.30)
        #plt.savefig(str(i))
        plt.grid()
        plt.show()
        print('Event Start: ' + str(mStart) + ', Precise Starts: ' + str(pStart1) + ', ' + str(pStart2) + ', diff: ' + str(pStart1-pStart2) + ', MEANSTD: ' + str(AcousticCut.std().mean()))
