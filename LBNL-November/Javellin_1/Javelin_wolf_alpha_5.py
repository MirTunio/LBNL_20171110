...
#%%
#
PreciseLog = SecondPass(MacroLog.sample(20), AcousticData, tdms_files[0])
...
#17 cutoff somehow, and then get rid of the shitty 
#%% NOTES
'''
Immediate modification, think about using a faux start at the max of the range
^NAICE

LF noise is really there, it will need to removed

SO THE FUNCTION AHenv works, filter for events with factor >17
'''
#%%
for i in np.arange(len(Look)):

    row = Look.iloc[i]
    strt = row.Start
    stop = row.Stop
    sample =  AcousticData.iloc[strt:stop]
    sample.plot(alpha=0.6)
    print(np.count_nonzero(sample.diff().abs()>test.abs()))