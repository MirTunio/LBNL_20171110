...
AcousticData = getData(filename,AcousticIndexes,TriggerIndex,Trigger_Threshold,manual=False)
...
#UberEventLog = ThirdPass(PreciesEventLog,AcousticData,filename[:-5])
#print(PreciseEventLogSample)
#PlotAll(AcousticData,PreciseEventLogSample,EnvLog,PreciseCutOff,env_test)

PreciseEventLog,EnvLog = SecondPass(MacroEventLog,AcousticData,filename[:-5])
PreciseEventLog.to_csv('Events_' + filename[:-5]+'.csv')
